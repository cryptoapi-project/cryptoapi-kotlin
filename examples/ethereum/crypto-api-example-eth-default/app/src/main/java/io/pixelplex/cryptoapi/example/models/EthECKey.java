...
import io.pixelplex.bitcoinjexample.ethereum_components.ECSignatureFactory;
 ...
 */
public class EthECKey implements Serializable {
     ...
     */
    public EthECKey(Provider provider, SecureRandom secureRandom) {
     ...
     * @param secureRandom -
     */
    public EthECKey(SecureRandom secureRandom) {
     ...
     * All private key operations will use the provider.
     */
    public EthECKey(Provider provider, PrivateKey privKey, ECPoint pub) {
        this.provider = provider;
     ...
     * BouncyCastle will be used as the Java Security Provider
     */
    public EthECKey(BigInteger priv, ECPoint pub) {
        this(
     ...
     * @return  -
     */
    public static EthECKey fromPrivate(BigInteger privKey) {
        return new EthECKey(privKey, CURVE.getG().multiply(privKey));
    }
     ...
     * @return -
     */
    public static EthECKey fromPrivate(byte[] privKeyBytes) {
     ...
     * @return  -
     */
    public static EthECKey fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new EthECKey(priv, pub);
    }
     ...
     * @return -
     */
    public static EthECKey fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        check(priv != null, "Private key must not be null");
        check(pub != null, "Public key must not be null");
        return new EthECKey(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @return -
     */
    public static EthECKey fromPublicOnly(ECPoint pub) {
        return new EthECKey(null, pub);
    }
     ...
     * @return -
     */
    public static EthECKey fromPublicOnly(byte[] pub) {
        return new EthECKey(null, CURVE.getCurve().decodePoint(pub));
    }
    ...
    public static byte[] computeAddress(byte[] pubBytes) {
        ...
        keccak.update(pubBytes, 0, pubBytes.length);
        pubBytes = keccak.digest();
        return Arrays.copyOfRange(pubBytes, 12, pubBytes.length);
    }
     ...
     */
    public static EthECKey fromNodeId(byte[] nodeId) {
        ...
        return EthECKey.fromPublicOnly(pubBytes);
    }
     ...
     * @return ECKey
     */
    public static EthECKey signatureToKey(byte[] messageHash, String signatureBase64) throws SignatureException {
        final byte[] keyBytes = signatureToKeyBytes(messageHash, signatureBase64);
        return EthECKey.fromPublicOnly(keyBytes);
    }
     ...
     * @return ECKey
     */
    public static EthECKey signatureToKey(byte[] messageHash, ECDSASignature sig) throws SignatureException {
...
//            logger.error("Caught NPE inside bouncy castle", npe);
     ...
     * @return ECKey
     */
    public static EthECKey recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig, messageHash);
        if (pubBytes == null) {
            return null;
        } else {
    ...
    public EthECKey decompress() {
        if (!pub.isCompressed())
            ...
            return new EthECKey(this.provider, this.privKey, decompressPoint(pub));
    }
    ...
    public EthECKey compress() {
        if (pub.isCompressed())
            ...
            return new EthECKey(this.provider, this.privKey, compressPoint(pub));
    }
    ...
    public boolean verify(byte[] data, byte[] signature) {
        return EthECKey.verify(data, signature, getPubKey());
    }
    ...
    public boolean verify(byte[] sigHash, ECDSASignature signature) {
        return EthECKey.verify(sigHash, signature, getPubKey());
    }
